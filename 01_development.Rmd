...
# add to Annie Swafford examples
ase <- c( 
...
myscores <- c(0, 0.75, 1, 0, 1, 1, 0.75, 0, -1, -1, 0, 0, 1) 
myaid = paste("sent_", 
                     c("01", "02", "03", "04", "05", "06", 
...
Use 4 best to model sentiment directions. Where confidence interval overlaps zero 
then is neutral.
```{r sentiment-direction-ensemble, fig.width=7, fig.height=4}
bind_cols(dftest, s_hu[, c('word_count', 'ave_sent_huliu')], 
          s_rinker[, 'ave_sent_rinker'], s_jockers[, 'ave_sent_jockers'],
          out1[, 'ave_sent_stanford']) %>% 
