...
In the end obtained 806 DOIS and 517 Abstracts for 1147 vertebrate studies from the Conservation Evidence database.
...
cebird %>% 
  ...
  select(study.id, publication.year,publication.title, journal.name, doi, url, doi_clean, doi_url, ab_clean, intervention_count) -> t2
# bind together
bind_rows(t1, t2) %>% 
  mutate(ab_clean = str_squish(str_trim(str_to_upper(ab_clean)))) -> out_doi_ab
# how many?
out_doi_ab %>% filter(!is.na(doi_clean)) %>% nrow() #806 DOIS
out_doi_ab %>% filter(!is.na(ab_clean)) %>% nrow() # 517 Abstracts
# Export  
...
ab_s03 <- plyr::adply(scopus_toget[1:25, ], .margins = 1, .fun = get_scopus_abdoi)
ab_s04 <- plyr::adply(scopus_toget[26:35, ], .margins = 1, .fun = get_scopus_abdoi)
ab_s05 <- plyr::adply(scopus_toget[36:44, ], .margins = 1, .fun = get_scopus_abdoi)
ab_s06 <- plyr::adply(scopus_toget[46:48, ], .margins = 1, .fun = get_scopus_abdoi)
ab_s06 <- plyr::adply(scopus_toget[49, ], .margins = 1, .fun = get_scopus_abdoi)
...
out_doi_ab_scopus %>% left_join(scopus_abs02) %>% 
 ...
 # how many?
out_doi_ab_scopus02 %>% filter(!is.na(doi_clean)) %>% nrow() # 843 DOIS
out_doi_ab_scopus02 %>% filter(!is.na(ab_clean)) %>% nrow() # 828 Abstracts
