...
ab04 <- plyr::adply(cedat_alt[601:777, ], .margins = 1, .fun = get_abstracts)
#join
...
source("R/safe_pubmedfetch.R")
source("R/get_pubmedids.R")
source("R/get_pubmed_abstracts.R")
...
abs_crossref %>% left_join(abs_pub) %>% 
  mutate(ab_clean = coalesce(AB, ab_pubmed, ab_text)) -> abs_all
# 266 Abstracts
abs_all %>% 
  filter(!is.na(ab_clean))
```
