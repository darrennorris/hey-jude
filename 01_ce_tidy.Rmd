...
In the end obtained 839 DOIS and 927 Abstracts for 1107 vertebrate articles from the Conservation Evidence database (total of 1145 studies including grey literature).
Next step - search with rscopus using titles.
...
cedat %>% 
  ...
  ungroup() -> cedat_scraped
cedat_scraped %>% nrow() #1094
# 35 not journals
cedat_scraped %>% 
...
cedat_scraped %>% 
  filter(!is.na(doi)) %>% nrow()
# Which journals have most articles
cedat_scraped %>% 
...
Join scraped CE and birds. Total of 1145 studies.
```{r join-studies}
sid <- as.character(unique(cedat_scraped$study.id))
bind_rows(cedat_scraped %>% 
            # exclude book chapter
            filter(!study.id==7468),
cebird %>% 
  ...
  select(study.id, publication.year,publication.title, journal.name, doi, url, intervention_count) 
) -> cedat_studies
```

## Get dois from CE data.
```{r get-dois}
# Create new doi column to join with bibliometrics etc
# clean out prefixes
...
```
Check Abstract with no full stops for study 8144.

Get dois and Abstracts from pii and eid.
```{r get-doi-scopus}
# Helper functions
source("R/safe_scopus_ab.R")
source("R/get_scopus_ab.R")
source("R/get_scopus_abeid.R")
...
filter(is.na(doi_ce) ,sci_flag ==1) %>% 
  ...
  select(study.id, id_pii) -> pii_toget # 37
cedat_study02 %>% 
  mutate(eid_flag = ifelse(str_detect(doi_url, "eid"), 1, 0), 
         pii_flag = ifelse(str_detect(doi_url, "pii"), 1, 0)) %>%
filter(is.na(doi_ce) ,eid_flag ==1, pii_flag==0) %>% 
  separate(doi_url, into=c("a", "id_eid"), sep="eid=") %>% 
  select(study.id, id_eid) %>% 
  separate(id_eid, into=c("id_eid", NA), sep="&") -> eid_toget #9

ab_pii01 <- plyr::adply(pii_toget, .margins = 1, .fun = get_scopus_ab)
ab_eid01 <- plyr::adply(eid_toget, .margins = 1, .fun = get_scopus_abeid)
bind_rows(
ab_pii01 %>% 
  select(study.id, doi_scopus, ab_scopus), 
ab_eid01 %>% 
  select(study.id, doi_scopus, ab_scopus)) -> scopus_abs
nrow(scopus_abs) # 46
length(unique(scopus_abs$doi_scopus)) # 46


```

Update studies with dois and Abstracts.
```{r update-studies}

cedat_study02 %>% left_join(scopus_abs %>% 
                              filter(!is.na(doi_scopus))) %>% 
  mutate(doi_clean = doi_scopus, 
    ab_clean = str_squish(str_trim(ab_scopus))) %>% 
  select(!doi_scopus, !ab_scopus) %>%
  ...
  mutate(doi_url = ifelse(url_flag==1 | wos_flag==1 | www_flag==1, NA_character_, doi_url)) %>% 
  mutate(doi_clean = coalesce(doi_clean, doi_ce, doi_url)) %>% 
  separate(doi_clean, into=c("doi_clean", NA), sep=" ") %>% 
  mutate(doi_clean = ifelse(doi_clean =="10.7325/7", NA, doi_clean)) %>% 
  select(!c(ab_scopus, doi_scopus, url_flag, wos_flag, www_flag)) -> cedat_study03
  
cedat_study03 %>% 
  filter(!is.na(doi_clean), is.na(ab_clean))  %>% 
  filter(!doi_clean =="10.7325/7") %>% 
  distinct() -> cedat_tidy

cedat_tidy %>% nrow() #797
length(unique(cedat_tidy$doi_clean)) # 797
length(unique(cedat_tidy$study.id)) #797

```

Try rscopus for all dois missing Abstracts.
```{r get-abs-scopus}
...
# Run. Approximately 15 minutes. tea time.
scopus_ab01 <- plyr::ddply(cedat_tidy[1:200, ], .(study.id), .fun = get_scopus_abdoi)
scopus_ab02 <- plyr::ddply(cedat_tidy[201:400, ], .(study.id), .fun = get_scopus_abdoi)
scopus_ab03 <- plyr::ddply(cedat_tidy[401:600, ], .(study.id), .fun = get_scopus_abdoi)
scopus_ab04 <- plyr::ddply(cedat_tidy[601:797, ], .(study.id), .fun = get_scopus_abdoi)

bind_rows(scopus_ab01, scopus_ab02, scopus_ab03, scopus_ab04) -> scopus_abs02

```

Export results
```{r export-results}
# 821 Abstracts and 839 dois
cedat_study03 %>% 
  left_join(scopus_abs02) %>% 
  mutate(doi_clean = str_trim(coalesce(doi_clean, doi_scopus)), 
         ab_clean = str_trim(coalesce(ab_clean, ab_scopus))) %>% 
...
#821 distinct Abstracts
out_cedat_dois %>% filter(!is.na(ab_clean)) %>% select(ab_clean) %>% 
  distinct()
# all dois and abstracts are unique
length(na.omit(out_cedat_dois$ab_clean)) # 821
length(unique(na.omit(out_cedat_dois$ab_clean))) # 821
length(unique(na.omit(out_cedat_dois$doi_clean))) # 839
length(na.omit(out_cedat_dois$doi_clean)) # 839
...
Check results
```{r}
# 23 Journals to manually fill missing dois
# no doi and url is .pdf (ICES Marine = 7)
# no doi and no url (Biological Conservation = 5)
# invalid links e.g. wos / article (Oryx = 4)
out_cedat_dois %>% 
  filter(!is.na(journal.name)) %>% 
...
Journals with most NA
```{r}
# Following journals do not have dois. 
# No doi = no altimetric.......
# Total of 153 studies to be manually included.
...
### Abstracts via titles
```{r abstracts-from-titles}
# helper functions
source("R/safe_scopus_search.R")
source("R/get_scopus_search.R")

out_cedat_dois %>% 
  filter(!is.na(journal.name), is.na(ab_clean)) %>%
  #nrow() # 286 
  select(study.id, publication.title, journal.name) %>% 
  mutate(title_clean = str_squish(str_trim(textclean::replace_html(publication.title, symbol = FALSE))) 
        ) -> cedat_titles
# Cant track down what causes warning but this works. 
scopus_search01 <- plyr::ddply(cedat_titles, .(study.id), .fun = get_scopus_search)


# Below are cases to solve
# 1) return multiple articles
cedat_titles %>% filter(study.id==6352) -> testdf
t1 <- safe_scopus_search(testdf[, 'title_clean'], view = "COMPLETE",
                             verbose = FALSE, headers = insttoken)
t1_df <- gen_entries_to_df(t1$result$entries)$df

t1_df %>% 
  mutate(title_flag = if_else(str_detect(`dc:title`, 
                                         fixed("Hoolock gibbon conservation in India")),1,0)) %>% filter(title_flag==1) -> t2_df
nrow(t2_df)

# 2) search when titles have special characters/accents 
# two examples 
cedat_titles %>% filter(study.id==10374) %>% pull(title_clean) -> test_simple
cedat_titles %>% filter(study.id==10385) %>% pull(title_clean) -> test_accent

test_simple_query <- paste("TITLE(",test_simple,")", sep="")
test_simple_query02 <- "TITLE(Over-exploitation and (illegal) trade of reptiles in Indonesia)"
test_simple_query03 <- "TITLE(Over-exploitation and (illegal) trade of reptiles in Indonesia (Duméril & Bibron, 1844))" # bad request
test_simple_query04 <- "TITLE(Over-exploitation and (illegal) trade of reptiles in Indonesia (Duméril) & Bibron, 1844)" # bad request

# something in (Dumeril & Bibron, 1844) generates bad request.
test_accent_query <- paste("TITLE(",test_accent,")", sep="") # 400 Bad request
test_accent_query02 <- paste("TITLE(", "Captive husbandry and reproduction of the Madagascan tree boa Sanzinia madagascariensis (Duméril & Bibron, 1844)" ,")", sep="") # Bad request
test_accent_query03 <- paste("TITLE(", "Captive husbandry and reproduction of the Madagascan tree boa Sanzinia madagascariensis (Duméril Bibron, 1844)" ,")", sep="") # Bad request
test_accent_query04 <- paste("TITLE(", "Captive husbandry and reproduction of the Madagascan tree boa Sanzinia madagascariensis (Dumeril Bibron, 1844)" ,")", sep="") # Bad request
test_accent_query05 <- paste("TITLE(", "Captive husbandry and reproduction of the Madagascan",")", sep="") # short version works
test_accent_query06 <- paste("TITLE(", "Captive husbandry and reproduction of the Madagascan tree boa Sanzinia madagascariensis" ,")", sep="") # OK
title_search <- rscopus::scopus_search(test_accent_query06, view = "COMPLETE",  headers = insttoken, verbose = FALSE)
res_df <- gen_entries_to_df(title_search$entries)$df
ab_out <- res_df$`dc:description`


```

Join Abstracts from titles
```{r join-abstracts}
# to check if we got the right article
out_cedat_dois %>% left_join(scopus_search01) -> abs_check
# Same. Differences due to html format codes etc
abs_check %>% filter(!is.na(ab_scopus)) %>% 
  mutate(flag_same = ifelse(publication.title==title_scopus,1,0)) %>% 
  filter(flag_same==0) %>% 
  select(publication.title,title_scopus)


out_cedat_dois %>% left_join(scopus_search01) %>% 
  mutate(ab_clean = coalesce(ab_clean, ab_scopus)) %>% 
  select(study.id, publication.year,publication.title, journal.name, doi, url, doi_clean, doi_url, ab_clean, intervention_count) -> out_cedat_dois2

# all dois and abstracts are unique
length(na.omit(out_cedat_dois2$ab_clean)) # 927
length(unique(na.omit(out_cedat_dois2$ab_clean))) # 927
length(unique(na.omit(out_cedat_dois2$doi_clean))) # 839
length(na.omit(out_cedat_dois2$doi_clean)) # 839
...
Code examples using WOS search results, pubmed, crossref and results from Zhang search.


Now join with Abstracts from an earlier WOS search. 117 join at first attempt. 
