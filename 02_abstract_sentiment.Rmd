...
study_abstracts_class %>% 
  filter(!is.na(word_class)) %>%
  slice_sample(n=9, by=strat_class, replace = FALSE) -> study_abstracts_105

length(unique(study_abstracts_105$mykey)) #105
study_abstracts_105 %>% pull(mykey) -> sample_105
plot(sort(study_abstracts_105$ave_sentiment))
study_abstracts_105 %>% 
  arrange(ave_sentiment) %>% 
  mutate(rank = row_number()) %>%
  ggplot(aes(x= rank, y=ave_sentiment)) + 
  geom_point(aes(colour=sent_class, size=word_count))
```
