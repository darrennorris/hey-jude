...
Look at scores for simple phrases.
```{r simple-sentiment}

mysentences_pos <- c("Take a sad song and make it better.", 
                     "I am very pleased with it so far."
                 )
# Annie Swafford examples
ase <- c(
...
mysentiments <- c("positive", "positive", "neutral", "positive", "positive", "positive", 
                  "negative", "negative", "negative", "neutral", "neutral")
dftest <- data.frame(sentence = c(mysentences_pos, ase), sentiment = mysentiments)
...
bind_cols(dftest, s_hu[, c('word_count', 'ave_sent_huliu')], 
          s_rinker[, 'ave_sent_rinker'], 
          out1[, 'ave_sent_stanford'],
          sa %>% select(WordCount, SentimentGI, SentimentLM, SentimentQDAP) 
) %>% 
  select(sentence, sentiment, word_count, ave_sent_huliu, ave_sent_rinker, 
         ave_sent_stanford, SentimentGI, SentimentLM, SentimentQDAP) %>% 
  pivot_longer(cols=c(ave_sent_huliu, ave_sent_rinker, ave_sent_stanford, SentimentGI, SentimentLM, SentimentQDAP)) %>% 
...
```
Ensemble approach with three best dictionaries.
...
bind_cols(dftest, s_hu[, c('word_count', 'ave_sent_huliu')], 
  ...
  select(sentence, sentiment, word_count, ave_sent_huliu, ave_sent_rinker, 
         ave_sent_stanford) %>% 
  pivot_longer(cols=c(ave_sent_huliu, ave_sent_rinker, ave_sent_stanford)) %>% 
  group_by(sentiment, sentence) %>% summarise(med_sent = median(value)) %>%
  ggplot(aes(x=factor(sentiment), y = med_sent)) + 
