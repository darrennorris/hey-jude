...
cedat %>% 
  ...
  mutate(ab_clean = replace_non_ascii(ab_clean)) -> cedat_clean
...
cedat_clean %>%
    ...
    sentiment_by(ab_sentences, list(study.id), 
                 polarity_dt = lexicon::hash_sentiment_jockers_rinker, 
                         question.weight = 0) -> ceab_sentiment_jr
# For individual sentences.
uncombine(ceab_sentiment_jr) %>% 
  rename("sent_jr"="sentiment") ->  ceab_sentiment_sentences_jr
...
Join results from different lexicons.
```{r join-lexicons}
# Doubke check order has not changed.
bind_cols(cedat_sentences, sent_stanford[,'ave_sent_stanford'], 
          ceab_sentiment_sentences_jr[,'sent_jr'], 
          ceab_sentiment_sentences_lh[,'sent_lh'], 
          ceab_sentiment_sentences_j[,'sent_j']) -> sentiment_scores
