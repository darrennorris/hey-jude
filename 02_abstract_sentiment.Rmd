...
# 835 Abstracts from search 2000, 2021, 2022. Exclude those not vertebrates
#myfile <- "data/scopus_835.bib"
#myrefs <- bibliometrix::convert2df(file = myfile, dbsource = "scopus", 
#                                     format = "bibtex") 
#write.csv(myrefs, "data/scopus_bib.csv", row.names = FALSE)
# 593
datsco <- read.csv("data/scopus_bib.csv") %>% 
  filter(vert==1)
```

Join data from CE, Williams and Scopus. 1747 Abstracts.
```{r join-data}
bind_rows(
cedat %>% 
...
datsco %>% 
  mutate(study.id = row_number(), 
         publication.year = PY,
         publication.title = str_to_sentence(TI), 
         journal.name = str_to_title(SO), 
         ab_clean = str_to_sentence(AB), 
         doi_clean = DI) %>%
  select(study.id, publication.year, publication.title, journal.name, ab_clean, 
         doi_clean) %>% 
  mutate(source = "Scopus", mykey = paste("Scopus",study.id, sep="_"))
) -> study_abstracts
...
study_abstracts %>% 
  mutate(ab_clean = replace_html(ab_clean), 
         publication.title = replace_html(publication.title)) %>% 
  mutate(ab_clean = replace_non_ascii(ab_clean), 
         publication.title = replace_non_ascii(publication.title)) %>% 
  mutate(ab_clean = str_trim(str_squish(ab_clean)), 
         publication.title = str_trim(str_squish(publication.title))) -> abstracts_clean
# remove "â€™"  "â€˜" 'â€“' "â€"
...
# upwight negatyive and downweight 0
abstracts_clean %>%
    dplyr::mutate(ab_sentences = get_sentences(ab_clean))  %$%
    sentiment_by(ab_sentences, list(mykey), 
                 polarity_dt = lexicon::hash_sentiment_jockers_rinker, 
                 averaging.function = sentimentr::average_weighted_mixed_sentiment,
                         question.weight = 0) -> ab_sentiment_jr_mix

summary(ab_sentiment_jr$word_count)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   19.0   202.0   245.0   244.4   285.0  743.0 
summary(ab_sentiment_jr$ave_sentiment)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
# -0.54801  0.01714  0.10148  0.09886  0.18024  0.69254
summary(ab_sentiment_jr_mix$ave_sentiment)
#    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-1.45639 -0.22508 -0.02607 -0.07975  0.11501  0.69254

# For individual sentences.
uncombine(ab_sentiment_jr) %>% 
  rename("sent_jr"="sentiment") ->  ab_sentiment_sentences_jr
uncombine(ab_sentiment_jr_mix) %>% 
  rename("sent_jr"="sentiment") ->  ab_sentiment_sentences_jr_mix
summary(ab_sentiment_sentences_jr$word_count)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   1.00   15.00   22.00   22.6   29.00  111.00      94 

# Scores for positive and negative
...
dftest %>%
 ...
 #    abkey word_count        sd ave_sentiment
# 1:   neg        124 0.3490789    -0.3727813
# 2:   pos        150 0.1918053     0.5219342
dftest %>%
    dplyr::mutate(ab_sentences = get_sentences(ab_clean))  %$%
    sentiment_by(ab_sentences, list(abkey), 
                 polarity_dt = lexicon::hash_sentiment_jockers_rinker, 
                 averaging.function = sentimentr::average_weighted_mixed_sentiment,
                         question.weight = 0) -> ab_sentiment_test_mix 
#    abkey word_count        sd ave_sentiment
# 1:   neg        124 0.3490789    -1.4993570
# 2:   pos        150 0.1918053     0.5219342
...
abstracts_clean %>% 
  ...
  mutate(flag_pos = if_else(lcl>0,1,0), 
         ...
         sent_class = case_when(lcl > 0 ~"positive", 
                                ucl < 0 ~"negative", 
                                .default = "neutral")) %>% 
  filter(sent_class =="negative") %>% pull(ave_sentiment) %>% max() #[1] -0.1487846

abstracts_clean %>% 
  left_join(ab_sentiment_jr) %>% pull(ave_sentiment) %>% quantile() 
#    0%        25%        50%        75%       100% 
#-0.5480078  0.0171393  0.1014832  0.1802385  0.6925363 
abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% pull(ave_sentiment) %>% quantile()
#  0%         25%         50%         75%        100% 
#-1.45639327 -0.22508283 -0.02606893  0.11500656  0.69253627 
# n = 235 (20.2 %) negative on average 
abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% pull(ave_sentiment) %>% quantile(prob=0.10) -> neg_10
abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% pull(ave_sentiment) %>% quantile(prob=0.90) -> pos_10
abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% 
  # no all negative
  #mutate(lcl = ave_sentiment - (1.96*sd)) %>% 
  filter(ave_sentiment <= neg_25) %>% nrow()/nrow(abstracts_clean) -> prop_negative

abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% pull(ave_sentiment) %>% 
  quantile(probs = (1-prop_negative)) -> quant_pos
# n = 28 with postive >= quant_pos
abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% filter(ave_sentiment >= quant_pos) %>% nrow()
...
# n = 6 * 18 = 108 abstracts
abstracts_clean %>% 
  left_join(ab_sentiment_jr_mix) %>% 
  filter(word_count<=500) %>%
  mutate(word_class = case_when(word_count >50 & word_count <=150 ~"150", 
                                word_count >= 151 & word_count <=200  ~"200", 
                                word_count >= 201  & word_count <=250  ~"250", 
                                word_count >= 251  & word_count <=300  ~"300", 
                                word_count >= 301  & word_count <=350  ~"350", 
                                word_count > 350 ~ "long",
                                .default = NA_character_), 
         sent_class = case_when(ave_sentiment <= neg_10 ~"negative", 
                                ave_sentiment >= pos_10 ~"positive", 
                                .default = "neutral")) %>% 
...
study_abstracts_class %>% 
  ...
  ggplot(aes(x= ave_sentiment, y=rank)) + 
  geom_point(aes(fill=sent_class, size=word_count), shape=21, 
             alpha=0.3) + 
  scale_fill_viridis_d("sentiment") + 
  scale_size_continuous("word count") + 
...
study_abstracts_class %>% 
  filter(!is.na(word_class)) %>%
  slice_sample(n=6, by=strat_class, replace = FALSE) -> study_abstracts_sample

length(unique(study_abstracts_sample$mykey)) #108
study_abstracts_sample %>% pull(mykey) -> sample_studies # 108

study_abstracts_sample %>% 
  arrange(ave_sentiment) %>% 
  mutate(rank = row_number()) %>%
  ggplot(aes(x= ave_sentiment, y=rank)) + 
  geom_point(aes(fill=sent_class, size=word_count), shape=21, 
...
```{r}
study_abstracts_class %>% 
  mutate(flag_sample = if_else(mykey %in% sample_studies, "sample", "Abstracts")) %>%
  ggplot(aes(x=word_count)) + 
  geom_histogram(bins=8) +
  facet_wrap(~sent_class, scales = "free_y")

```
```{r}
study_abstracts_class %>% 
  mutate(flag_sample = if_else(mykey %in% sample_studies, "sample", "Abstracts")) %>%
  filter(flag_sample=="sample") %>%
  ggplot(aes(x=word_count)) + 
  geom_histogram(bins=8) +
  facet_wrap(~sent_class, scales = "free_y")
...
study_abstracts_class %>% 
  mutate(flag_sample = if_else(mykey %in% sample_studies, "sample", "Abstracts")) %>%
  filter(flag_sample=="sample") %>% pull(sent_class) %>% table()
# negative  neutral positive 
#      36       36       36 
```
...
study_abstracts_class %>% 
  mutate(flag_sample = if_else(mykey %in% sample_studies, 1, 0)) %>% 
