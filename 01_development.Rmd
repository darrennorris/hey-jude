...
dfsyuzhet <- data.frame(syuzhet_syuzhet=sent_syuzhet)
...
bind_cols(dftest, s_hu[, c('word_count', 'ave_sent_huliu')], 
          ...
          out1[, 'ave_sent_stanford'], dfsyuzhet %>% select(syuzhet_syuzhet),
          sa %>% select(WordCount, SentimentGI, SentimentLM, SentimentQDAP) 
) %>% 
  mutate(aid = paste("sent_", 1:11, sep="")) %>%
  select(aid, sentence, sentiment, word_count, ave_sent_huliu, ave_sent_rinker, ave_sent_jockers,
         syuzhet_syuzhet, ave_sent_stanford, SentimentGI, SentimentLM, SentimentQDAP) %>% 
  pivot_longer(cols=c(ave_sent_huliu, ave_sent_rinker, ave_sent_jockers, ave_sent_stanford, syuzhet_syuzhet, SentimentGI, SentimentLM, SentimentQDAP)) %>% 
