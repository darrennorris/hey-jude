...
all_alti_senti %>% 
  filter(!is.na(AB), !is.na(DI), !is.na(altmetric), PY %in% study_years) %>% 
  arrange(TC) %>% 
  mutate(cite_order = rank(TC)) %>% 
  ggplot(aes(x=altmetric, y=cite_order)) + 
  geom_point(aes(col=ti_sentiment_med)) + 
  scale_colour_gradient2("title\nsentiment", 
                         low = "red", mid = "white", high = "green") +
  facet_wrap(~PY, scales = "free", ncol = 1) + 
  labs(title = "(A)", y="rank of times cited") -> fig_citation
...
all_alti_senti %>% 
  ...
  labs(title = "(B)", x="abstract sentiment", 
       y="rank title sentiment") -> fig_sent_cite
...
all_alti_senti %>% 
  ...
  scale_color_viridis_c("altmetric", trans = "log10") +
  facet_wrap(~PY, scales = "free", ncol = 1) + 
  labs(title = "(C)", x="abstract sentiment", 
       y="rank title sentiment") -> fig_sent_alt
