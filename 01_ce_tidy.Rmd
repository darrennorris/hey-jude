...
bind_rows(
cedat_studies %>% 
  left_join(abs_all %>% left_join(zhang_abs %>% select(study.id, AB) %>% distinct() %>%
                        rename("ABzhang"="AB")) %>% 
...
select(study.id, publication.year,publication.title, journal.name, doi, url, doi_clean, doi_url, ab_clean, intervention_count),

cebird %>% left_join(abs_brids %>% 
  rename("study.id"="ID") %>% 
...
out_doi_ab %>% filter (!is.na(ab_clean)) %>% nrow() # 518 Abstracts
# Export  
