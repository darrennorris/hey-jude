...
```{r load-data}
# 927 Abstracts
cedat_clean <- read.csv("data/cedat_cleandois.csv") %>% 
...
cedat_clean %>% 
  mutate(ab_02 = replace_html(ab_clean) %>%
    dplyr::mutate(ab_split = get_sentences(ab_02))  %$%
    sentiment_by(ab_02, list(study.id)) -> ceab_sentiment

# Individual sentences. Start 17:02 17 March
cedat_clean %>% 
  mutate(ab_02 = replace_html(ab_clean) %>%
    dplyr::mutate(ab_split = get_sentences(ab_02)) %>% 
  unnest_longer(ab_split) -> ab_split

ab_split %$%
    sentiment_by(ab_split) -> ceab_sentances_sentiment
