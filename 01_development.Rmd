...
Take a look at sentiment stuff
```{r join-abstract-sentiment, eval=FALSE}
# summarise sentiment by abstract
ab_sent_senti <- readRDS("data/ab_sent_senti.rds")
ab_sent_senti %>% 
  group_by(DI, AB) %>% 
  summarise(ab_word_count = sum(word_count), 
            ab_sentance_count = n(), 
            sentiment_min = round(min(ave_sentiment),4), 
            sentiment_max = round(max(ave_sentiment),4), 
            sentiment_med = round(median(ave_sentiment),3),
            sentiment_sd = round(sd(ave_sentiment),3)) -> ab_sent_senti
# add sentiment to abstracts
alt_all %>% 
  left_join(ab_sent_senti) -> all_alti_senti
...
Plot to see.
```{r plot-sentiment-year, fig.height=5, fig.width=4}
all_alti_senti %>% 
  filter(!is.na(AB), !is.na(DI), !is.na(altmetric), PY %in% study_years) %>%
  ggplot(aes(x=factor(PY), y=sentiment_med)) + 
  geom_point() + 
